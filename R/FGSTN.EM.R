FGSTN.EM <- function(y, xi, s, la, nu, iter.max=200, tol=10^-6){
...
# E step
	...
	LL <- sum(log(2/s*dt(y.xi/s,nu)*pnorm(alp.a.y.xi))) # log-likelihood function
# MCE steps
	xi <-  optim(xi,function(x){
	y.xi <- y-x
	a.y.xi <- t(outer(y.xi,a,'^'))
	alp.a.y.xi <- as.numeric(t(alp)%*%a.y.xi)
	return(-sum(log(2/s*dt(y.xi/s,nu)*pnorm(alp.a.y.xi))) )
		},method= "BFGS")$par
	y.xi <- y-xi
	...
	nu <- optim(nu,function(x){
		-sum(log(2/s*dt(y.xi/s,x)*pnorm(alp.a.y.xi)))
		},method="L-BFGS-B",lower=.001,upper=Inf)$par
	LL.new <- sum(log(2/s*dt(y.xi/s,nu)*pnorm(alp.a.y.xi))) # log-likelihood function
	count <- count +1
	dif <- abs(LL.new/LL-1)
	}
